...
Furthermore it contains Writer classes that ease the process of writing
prediction masks, detection output, and a tils score.
"""
import json
...
def open_multiresolutionimage_image(path: Path) -> mir.MultiResolutionImage:
    ...
    Returns
    -------
    image: MultiEesolutionImage
         Opened multiresolution image

    Raises
    ------
    IOError
        Raises when opened image is None
    """
...
class SegmentationWriter:
    def __init__(
        ...
        spacing: tuple
            base spacing of the output image
        """
    ...
    def write_segmentation(
        ...
        self._writer.writeBaseImagePartToLocation(
            tile.flatten().astype('uint8'), x=int(x), y=int(y)
        )
